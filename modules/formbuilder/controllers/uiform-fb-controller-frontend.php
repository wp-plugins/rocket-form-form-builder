...
class Uiform_Fb_Controller_Frontend extends Uiform_Base_Module {
    ...
    protected function __construct() {
        ...
        $this->wpdb = $wpdb;
        //Shortcodes
        add_shortcode('rocketform', array(&$this, 'get_form_shortcode'));
    ...
    public function ajax_submit_ajaxmode() {
        ...
        if (!empty($_POST['_rockfm_submitted']) && intval($_POST['_rockfm_submitted']) > 0) {
            $resp['success'] = 1;
            $resp['show_message'] = (isset($_POST['_rockfm_onsubm_smsg'])) ? Uiform_Form_Helper::base64url_decode($_POST['_rockfm_onsubm_smsg']) :
    ...
    public function get_form_shortcode($attributes, $content = null) {
        ...
        if (isset($_POST['_rockfm_type_submit'])
            ...
            if (!empty($_POST['_rockfm_submitted']) && intval($_POST['_rockfm_submitted']) > 0) {
                echo (isset($_POST['_rockfm_onsubm_smsg'])) ? Uiform_Form_Helper::base64url_decode($_POST['_rockfm_onsubm_smsg']) : __('Success! your form was submitted', 'frocket_front');
            } else {
