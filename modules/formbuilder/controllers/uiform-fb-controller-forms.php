...
class Uiform_Fb_Controller_Forms extends Uiform_Base_Module {
    ...
    public function ajax_load_preview_form() {
        ...
        $form_id = ($_POST['form_id']) ? Uiform_Form_Helper::sanitizeInput(trim($_POST['form_id'])) : '';
        ...
        <link media="all" type="text/css" href="<?php echo UIFORM_FORMS_URL . '/assets/frontend/css/rockfm_form' . $form_id . '.css?' . date("Ymdgis"); ?>"
              ...
              rel="stylesheet">
        <?php
        echo do_shortcode('[rocketform id="' . $form_id . '"]');
        ?>
    ...
    public function ajax_save_form() {
        ...
        $_POST['form_data'] = str_replace("\'", "'", $_POST['form_data']);
        $fmb_data = (isset($_POST['form_data']) && $_POST['form_data']) ? array_map(array('Uiform_Form_Helper', 'sanitizeRecursive'), json_decode($_POST['form_data'], true)) : array();
        $data['fmb_data'] = json_encode($fmb_data);
        $data['fmb_data2'] = isset($fmb_data['onsubm']) ? json_encode($fmb_data['onsubm']) : '';
        $data['fmb_name'] = ($_POST['uifm_frm_main_title']) ? Uiform_Form_Helper::sanitizeInput(trim($_POST['uifm_frm_main_title'])) : '';
        $data['fmb_html_backend'] = ($_POST['form_html_backend']) ? Uiform_Form_Helper::sanitizeInput($_POST['form_html_backend']) : '';
        $data['created_ip'] = $_SERVER['REMOTE_ADDR'];
        $data['created_by'] = 1;
        $data['created_date'] = date('Y-m-d h:i:s');
    ...
    public function list_uiforms() {
        ...
        //list all forms
        $data = $config = array();
        $config['base_url'] = admin_url() . '?page=rocket-forms&mod=formbuilder&controller=forms&action=list_uiforms';
