...
class Uiform_Model_Fields {
    ...
    function queryGetListFieldsEnabled($form_id) {
        $query = sprintf('select f.fmf_uniqueid
            from %s f 
            where f.fmf_status_qu=1 and f.form_fmb_id=%s', $this->table, $form_id);
    ...
    function getFieldNameByUniqueId($uid, $form_id) {
        $query = sprintf('
            ...
            where
            frm.fmb_id=%s
            and f.fmf_uniqueid="%s"', $this->table, $this->tbformtype, $this->tbform, $form_id, $uid);
    ...
    function getDataByUniqueId($uid, $form_id) {
        $query = sprintf('
            ...
            ', $this->table, $this->tbform, $form_id, $uid);
        return $this->wpdb->get_row($query);
    }
