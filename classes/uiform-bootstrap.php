...
class Uiform_Bootstrap extends Uiform_Base_Module {
    ...
    public function register_hook_callbacks() {
        ...
        //load admin
        if (is_admin() && ( ( isset($_GET['page']) && $_GET['page'] == 'rocket-forms' ) || ( isset($_POST['page']) && $_POST['page'] == 'rocket-forms' ) )) {
...
JS;
    ...
    public function loadMenu() {
        ...
        add_menu_page('RForm - Form Builder', 'Rocket Forms Express', "edit_posts", "rocket-forms", array(&$this, "route_page"), UIFORM_FORMS_URL . "/assets/backend/image/rockfm-logo-ico.png");
        add_submenu_page("rocket-forms", __('List by form', 'FRocket_admin'), __('Records', 'FRocket_admin'), 'manage_options', "rocket_forms_record", array(&$this, 'load_data_option'));
        add_submenu_page("rocket-forms", __('Import', 'FRocket_admin'), __('Import', 'FRocket_admin'), 'manage_options', "rocket_forms_import", array(&$this, 'load_data_option'));
        add_submenu_page("rocket-forms", __('Export', 'FRocket_admin'), __('Export', 'FRocket_admin'), 'manage_options', "rocket_forms_export", array(&$this, 'load_data_option'));
        add_submenu_page("rocket-forms", __('Charts', 'FRocket_admin'), __('Charts', 'FRocket_admin'), 'manage_options', "rocket_forms_charts", array(&$this, 'load_data_option'));
        add_submenu_page("rocket-forms", __('Settings', 'FRocket_admin'), __('Settings', 'FRocket_admin'), 'manage_options', "rocket_forms_settings", array(&$this, 'load_data_option'));
    }

    public function load_data_option() {
        switch ((string) $_GET['page']) {
            case 'rocket_forms_record':
                $redirect = UIFORM_ADMINPATH . '?page=rocket-forms&mod=formbuilder&controller=records&action=info_records_byforms';
                break;
            case 'rocket_forms_import':
                $redirect = UIFORM_ADMINPATH . '?page=rocket-forms&mod=formbuilder&controller=forms&action=create_uiform&opt=import';
                break;
            case 'rocket_forms_export':
                $redirect = UIFORM_ADMINPATH . '?page=rocket-forms&mod=formbuilder&controller=forms&action=export_form';
                break;
            case 'rocket_forms_charts':
                $redirect = UIFORM_ADMINPATH . '?page=rocket-forms&mod=formbuilder&controller=records&action=view_charts';
                break;
            case 'rocket_forms_settings':
                $redirect = UIFORM_ADMINPATH . '?page=rocket-forms&mod=formbuilder&controller=settings&action=view_settings';
                break;
            default:
                $redirect = UIFORM_ADMINPATH . '?page=rocket-forms';
        }
    ...
    public static function load_admin_resources() {
        ...
        if (is_admin()) {
            ...
            wp_localize_script(self::PREFIX . 'admin', 'uiform_vars', array('url_admin' => admin_url(), 'url_plugin' => UIFORM_FORMS_URL));
            wp_enqueue_script(self::PREFIX . 'rockfm_js_global_frontend');
            wp_localize_script(self::PREFIX . 'rockfm_js_global_frontend', 'rockfm_vars', array('ajaxurl' => admin_url('admin-ajax.php'), 'imagesurl' => UIFORM_FORMS_URL . "/assets/frontend/images",'assetsurl' => UIFORM_FORMS_URL . "/assets"));
        }
    }
    ...
    public function i18n() {
        ...
        $locale = apply_filters('plugin_locale', get_locale(), 'rocket-forms');
